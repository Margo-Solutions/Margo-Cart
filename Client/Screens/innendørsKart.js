...
export default function InnendørsKart({route}) {
  ...
  const { handleliste, Listcoords } = useContext(handContext); // henter ut handleliste fra context
  const [vare_navn, setVare_navn ] = useState([]);  // varene som brukeren skal hente ut i fra handlelisten dems 
  const [vare, setVare] = useState(["eple", "banan", "pære"]); // vare vis vi ikke får inn en handleliste 
...
const varetelling = () => {
  ...
  if (temp <= 0) {
    setStartCol(13);
    setStartRow(1);
    setEndRow(butikkvarer[temp].x);
    setEndCol(butikkvarer[temp].y);
    console.log("start: " + node_start_row +" "+ node_start_col + " end: " + node_end_row + " " + node_end_col);
  } else {
    setStartRow(butikkvarer[temp - 1].x);
    setStartCol(butikkvarer[temp - 1].y);
    setEndRow(butikkvarer[temp].x);
    setEndCol(butikkvarer[temp].y);
    console.log("start: " + node_start_row +" "+ node_start_col + " end: " + node_end_row + " " + node_end_col);
  }
  ...
  <View style={styles.container}>
      <View style={styles.line} />
        ...
        <TextInput 
          ...
          <Text style={{fontSize: 20, textAlign: 'center', color: '#120438'}}>Nå: {handleliste[vareNr].antall} {vare_navn[vareNr]}</Text>
